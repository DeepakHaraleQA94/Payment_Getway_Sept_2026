...
export default function Reports() {
  ...
  const runNow = async () => {
    setBusy(true);
    try {
      await api.post("/reports/scheduled/run", {}, { params: { tenant_id: selectedTenantId } });
  ...
  return (
    <div data-testid="reports-page">
      <PageHeader
        ...
        subtitle="Daily payments & settlements reports, generated automatically at 08:00 UTC."
        action={<Button data-testid="run-report-button" onClick={runNow} disabled={busy}><Play className="h-4 w-4 mr-2" /> Run report now</Button>}
      />
      <Panel className="mb-6 flex items-start gap-3">
        <FileText className="h-5 w-5 text-primary mt-0.5" />
        <div>
          <p className="text-sm">Reports are generated and stored for in-app download. Email delivery uses a
